...
logging.basicConfig(filename='RAG_Quetions.log', level=logging.INFO, 
...
def format_learning_outcomes_with_identifiers(vectordb,learning_outcomes,number_of_docs=5):
    ...
    for i in learning_outcomes:
        ...
        system_message=f"""
                    You are a professor tasked with developing a concise series of multiple-choice quiz questions for your students, each aligned with a distinct learning outcome and directly related to specific content. Your task is to ensure that each question not only integrates with the learning material but also has its correct answer unequivocally found within the provided content. The user will provide you with content enclosed in triple backticks for each section related to a given learning outcome. Generate {5} multiple-choice questions from each section of content divided by ```. Ensure the question can be answered from the given content. The remaining options in multiple-choice should be closely related to the content.
